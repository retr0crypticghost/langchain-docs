...
- Rejected insecure default JWT secret at startup for Agent Builder, enhancing security.
- Ensured Eppo control panel renders above Polly in the frontend for improved user experience.
- Standardized dropdown width and height in Bedrock and Azure model config in the playground.
- Updated prompt owners dialog for full backward compatibility in the frontend.
- Included per-annotator feedback detail in experiments CSV export for enhanced data insights.
- Added a new GET /me/slack_id endpoint in Agent Builder.
- Implemented Slack authorization with is_user_permitted_for_agent in Agent Builder.
- Added strict validation to prebuilt evaluator schemas and automated sync on model switch in the frontend.
- Fixed a generator bug on frontend related to custom Slack bot integration.
- Improved HTTP authentication security in message_received.py.
- Added forge settings page with an experimental UI for enhanced configurability.
- Enhanced UI by centering elements on the no access page.
- Refactored Consolidated multiple tooltips into one hover popover for better UI experience.
- Redesigned feedback chip popover for a modernized look.
- Added real-time sub-agent streaming UI in Agent Builder for dynamic updates.
...
- Fixed agent-builder to reject insecure default JWT secret at startup, enhancing security.
- Improved frontend by standardizing dropdown width and height in Bedrock and Azure model config for better UI consistency.
- Updated prompt owners dialog to be fully backwards compatible, enhancing user experience.
- Included per-annotator feedback detail in experiments CSV export for better data analysis.
- Added new API endpoint GET /me/slack_id to the agent builder for enhanced Slack integration.
- Implemented Slack authorization with is_user_permitted_for_agent in the agent builder.
- Updated frontend to add strict configuration to prebuilt evaluator schemas and synchronize on model switch.
- Fixed a bug in the frontend generator affecting custom Slack bot integration, improving reliability.
- Addressed potential HTTP auth leak in message_received.py for enhanced security.
- Improved user feedback by shielding feedback save from SAQ timeout cancellations.
- Added a new forge configurations CRUD API for better infrastructure management.
- Enhanced agent builder with a real-time sub-agent streaming UI, improving real-time monitoring.
- Fixed the frontend member search function in the owners dialog for better user experience.
- Added authz middleware to smith-go for better authorization handling.
- Redesigned feedback chip popover for improved UI experience.
...
- Rejected insecure default JWT secret at startup for agent-builder.
- Ensured Eppo control panel renders above Polly in the frontend.
- Standardized dropdown width and height in Bedrock and Azure model config in the playground.
- Fixed no default role id when RBAC is not enabled for workspace invites.
- Implemented new frontend prompt owners dialog with full backwards compatibility.
- Included per-annotator feedback detail in experiments CSV export.
- Added Slack disconnect to delete user integrations in agent builder.
- Added GET /me/slack_id endpoint in agent builder.
- Added Slack ID endpoint and reauth banner in agent builder.
- Implemented Slack authorization for agents in agent builder.
- Fixed frontend to add strict:true to prebuilt evaluator schemas and sync on model switch.
- Fixed potential HTTP auth leak in message_received.py.
- Added statement timeout at the database level.
- Replaced thread detail with smithdb endpoint.
- Added forge settings page with experimental UI.
- Fixed feedback save from SAQ timeout cancellation in smith-backend.
- Enhanced performance by making GitHub integration IDs nullable.
- Added flag to disable query shadowing.
- Proactively read current run improvements in smith-polly.
- Extended ColumnActions with isAlphabetical and onFilter props in the frontend.
- Redesigned feedback chip popover UI.
- Added real-time sub-agent streaming UI in the agent builder.
- Added new `trace_count_transactions_granular_rollup` table.
- Renamed "Use object in LangChain" to "Use Programmatically".
- Stopped channels identity modal from auto-saving in agent-builder.
