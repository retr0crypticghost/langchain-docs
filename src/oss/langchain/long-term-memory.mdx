...
Long-term memory lets your agent store and recall information across different conversations and sessions. Unlike [short-term memory](/oss/langchain/short-term-memory), which is scoped to a single thread, long-term memory persists across threads and can be recalled at any time.
...
- [Short-term memory](/oss/langchain/short-term-memory): Add conversation history within a single thread
- [Memory conceptual guide](/oss/concepts/memory): Learn about memory types (semantic, episodic, procedural) and strategies
- [Persistence](/oss/langgraph/persistence#memory-store): LangGraph store API reference and configuration
