...
For OAuth-protected servers, acquire a token externally and pass it via `headers`. The JSON config format does not support dynamic OAuth flows directly — for programmatic OAuth 2.1 with PKCE and automatic token refresh, use [`langchain-mcp-adapters` programmatically](/oss/python/langchain/mcp#oauth-21) with the MCP SDK's `OAuthClientProvider`.
