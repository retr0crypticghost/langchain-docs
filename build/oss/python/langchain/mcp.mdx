...
The `langchain-mcp-adapters` library uses the official [MCP SDK](https://github.com/modelcontextprotocol/python-sdk) under the hood. Any [`httpx.Auth`](https://www.python-httpx.org/advanced/authentication/) instance can be passed via the `auth` field on `sse` and `streamable_http` connections.

```python Custom httpx.Auth
from langchain_mcp_adapters.client import MultiServerMCPClient
...
The MCP SDK ships `OAuthClientProvider`, an `httpx.Auth` implementation that handles the full OAuth 2.1 authorization code flow with PKCE and automatic token refresh. Pass it directly to the `auth` field:

```python OAuth with OAuthClientProvider
from mcp.client.auth import OAuthClientProvider, TokenStorage  # [!code highlight]
...
# Implement token persistence (database, file, keychain, etc.)
...
oauth_auth = OAuthClientProvider(  # [!code highlight]
    server_url="https://mcp.example.com",  # [!code highlight]
    client_metadata=OAuthClientMetadata(  # [!code highlight]
        client_name="My LangChain App",  # [!code highlight]
        redirect_uris=[AnyUrl("http://localhost:3000/callback")],  # [!code highlight]
        grant_types=["authorization_code", "refresh_token"],  # [!code highlight]
        response_types=["code"],  # [!code highlight]
    ),  # [!code highlight]
    storage=MyTokenStorage(),  # [!code highlight]
    redirect_handler=my_redirect_handler,  # [!code highlight]
    callback_handler=my_callback_handler,  # [!code highlight]
)  # [!code highlight]
...
client = MultiServerMCPClient(
    {
        "protected-server": {
            ...
            "auth": oauth_auth,  # [!code highlight]
        }
    }
)
tools = await client.get_tools()
...
<Tip>
For a complete working example including redirect/callback handlers and in-memory token storage, see the [MCP SDK OAuth client example](https://github.com/modelcontextprotocol/python-sdk/blob/main/examples/snippets/clients/oauth_client.py).
</Tip>
